...
export function generateDeterministicBodies(n, seed, scenario = 'RANDOM') {
    rng.setSeed(seed);
    const masses = new Array(n);
    const state = new Float64Array(n * 4);
    if (scenario === 'BINARY') {
        // Two massive bodies
        masses[0] = 50000;
        state[0] = -200;
...
export function validateDeterminism(config) {
    const sim1 = new Simulator(generateDeterministicBodies(config.bodies, config.seed).state, createGravityDerivative(generateDeterministicBodies(config.bodies, config.seed).masses, config.epsilon), {
        dt: config.dt,
        epsilon: config.epsilon,
        integrator: config.integrator
    });
    const sim2 = new Simulator(generateDeterministicBodies(config.bodies, config.seed).state, createGravityDerivative(generateDeterministicBodies(config.bodies, config.seed).masses, config.epsilon), {
        dt: config.dt,
        epsilon: config.epsilon,
        integrator: config.integrator
    });
...
export function exportBenchmarkToFile(result) {
    const json = JSON.stringify(result, null, 2);
    if (typeof require !== 'undefined') {
        const fs = require('fs');
        fs.writeFile('benchmark-results.json', json, 'utf-8', (err) => {
            if (err)
                console.error('Error writing benchmark-results.json', err);
        });
    }
}
