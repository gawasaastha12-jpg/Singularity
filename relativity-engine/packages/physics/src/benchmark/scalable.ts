...
export interface ScalableBenchmarkResult {
    bodies: number;
    steps: number;
...
export function runScalableBenchmark(bodyCount: number, steps: number = 100, warmupSteps: number = 20): ScalableBenchmarkResult {
    const seed = 42;
    ...
    const stepTimes: number[] = new Array(steps);
    ...
    let totalTime = 0;
    for (let i = 0; i < steps; i++) totalTime += stepTimes[i];
...
export function runScalableSuite(): ScalableBenchmarkResult[] {
    const scale = [1000, 2000, 5000, 8000];
    const results: ScalableBenchmarkResult[] = [];
...
export function printScalableReport(results: ScalableBenchmarkResult[]): void {
