...
import { runBenchmarkSuite, exportBenchmarkToFile, BenchmarkSuite } from "./benchmark";

export interface BenchmarkAnalysis {
    fastestIntegrator: string;
    slowestIntegrator: string;
    rk4AvgStepTime1000: number;
    verletAvgStepTime1000: number;
    rk4Drift1000: number;
    verletDrift1000: number;
    scalingTrend: "quadratic" | "subquadratic" | "linear";
    maxDriftObserved: number;
    recommendedNextStep: string;
}

export function analyzeBenchmarkResults(filePath: string): BenchmarkAnalysis {
    const fs = require("fs");
    ...
    const data: BenchmarkSuite = JSON.parse(rawData);
    ...
    const rk4Times = new Map<number, number>();
    ...
    let trend: "quadratic" | "subquadratic" | "linear" = "quadratic";
    if (ratio < 2.5) {
        trend = "linear";
    } else if (ratio < 3.5) {
        trend = "subquadratic";
