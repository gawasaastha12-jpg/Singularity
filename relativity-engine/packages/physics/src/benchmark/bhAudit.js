...
import { audit_approximationsPerformed, audit_exactInteractions, resetAuditCounters } from '../forces/deterministicTraversal';
const SEED = 42;
...
function runDriftAudit(theta, steps, logInterval) {
...
function runDivergenceAudit(theta, steps) {
    const { state, masses } = generateDeterministicBodies(BODY_COUNT, SEED);
    const stateD = state;
    const stateBH = copyState(stateD);
...
function runDeterminismRecheck(theta, steps, interval) {
    const hashes1 = [];
    const hashes2 = [];
