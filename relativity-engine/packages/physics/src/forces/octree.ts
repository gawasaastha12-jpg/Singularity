...
export class Octree {
    ...
    public readonly centerX: Float64Array;
    public readonly centerY: Float64Array;
    public readonly centerZ: Float64Array;
    public readonly halfSize: Float64Array;

    public readonly mass: Float64Array;
    public readonly comX: Float64Array;
    public readonly comY: Float64Array;
    public readonly comZ: Float64Array;

    // child[nodeIndex * 8 + octant]
    public readonly child: Int32Array;
    // bodyIndex[nodeIndex] holds the inserted body id if leaf, or -1 if internal
    public readonly bodyIndex: Int32Array;
    ...
    public build(state: StateVector, masses: number[]): void {
        this.nodeCount = 0;
    ...
    private getOctant(nodeIdx: number, x: number, y: number, z: number): number {
        let octant = 0;
        if (x >= this.centerX[nodeIdx]) octant |= 1;
        if (y >= this.centerY[nodeIdx]) octant |= 2;
        if (z >= this.centerZ[nodeIdx]) octant |= 4;
        return octant;
    }

    private insert(bodyId: number, state: StateVector): void {
        const ix = bodyId * 4;
        const bx = state[ix];
        const by = state[ix + 1];
    ...
    private computeCOM(state: StateVector, masses: number[]): void {
    ...
    public exportBoundingBoxes(maxDepth: number, maxBoxes: number): Float32Array {
